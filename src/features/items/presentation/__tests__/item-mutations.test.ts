...
it('un alta sin conexión se guarda, se rehidrata en otro cliente y llega al servidor', async () => {
  ...
  const client = newClient()
  startPaused(client, itemMutationKeys.add, { communityId: 'c1', name: 'Pan', quantity: 2 })
  await settle()
  ...
  expect(repository.add).toHaveBeenCalledWith({ communityId: 'c1', name: 'Pan', quantity: 2 })
})
